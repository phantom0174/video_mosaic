...
#define CHECK_HIP(call) { const hipError_t error = call; if (error != hipSuccess) { std::cerr << "HIP Error: " << hipGetErrorString(error) << " at line " << __LINE__ << std::endl; exit(1); } }
#define CHECK_ROCBLAS(call) { if (call != rocblas_status_success) { std::cerr << "ROCBLAS Error at line " << __LINE__ << std::endl; exit(1); } }
...
void init_pinned_pool(size_t img_bytes) {
    ...
    for (int i = 0; i < pool_size; ++i) {
        unsigned char* ptr;
        hipHostMalloc(&ptr, img_bytes, hipHostMallocDefault);
...
void free_pinned_pool() {
    for (auto* ptr : g_pinned_pool) hipHostFree(ptr);
}
...
struct FrameResources {
    ...
    void allocate(size_t in_bytes, size_t out_bytes, int num_blocks, int pixel_dim, int loaded_tiles) {
        ...
        CHECK_HIP(hipHostMalloc(&h_pinned_input, in_bytes, hipHostMallocDefault));
        CHECK_HIP(hipHostMalloc(&h_pinned_output, out_bytes, hipHostMallocDefault));

        CHECK_HIP(hipMalloc(&d_raw_input_img, in_bytes));
        CHECK_HIP(hipMalloc(&d_input_blocks, num_blocks * pixel_dim * sizeof(float)));
        CHECK_HIP(hipMalloc(&d_gemm_result, loaded_tiles * num_blocks * sizeof(float)));
        CHECK_HIP(hipMalloc(&d_best_indices, num_blocks * sizeof(int)));
        CHECK_HIP(hipMalloc(&d_output_img, out_bytes));
    }

    void free() {
        hipHostFree(h_pinned_input); hipHostFree(h_pinned_output);
        hipFree(d_raw_input_img); hipFree(d_input_blocks);
        hipFree(d_gemm_result); hipFree(d_best_indices); hipFree(d_output_img);
...
public:
    bool load(int& out_count, std::vector<float>& features, std::vector<float>& sq_sums, std::vector<unsigned char>& pixels) {
        ...
        fread(features.data(), sizeof(float), features.size(), fp);
        fread(sq_sums.data(), sizeof(float), sq_sums.size(), fp);
        fread(pixels.data(), sizeof(unsigned char), pixels.size(), fp);
        fclose(fp);
        return true;
    }
};
...
int main() {
    ...
    CHECK_HIP(hipSetDevice(g_local_rank % num_gpus));
    if (g_rank == 0) std::cout << ">>> Hybrid Mosaic Generator (TurboJPEG + Multi-Stream) <<<" << std::endl;
    ...
    hipStream_t mem_stream; CHECK_HIP(hipStreamCreate(&mem_stream)); 

    rocblas_handle blas_handle; CHECK_ROCBLAS(rocblas_create_handle(&blas_handle));

    float *d_features, *d_tile_sq_sums; unsigned char *d_pixels;
    CHECK_HIP(hipMalloc(&d_features, h_features.size() * sizeof(float)));
    CHECK_HIP(hipMemcpyAsync(d_features, h_features.data(), h_features.size() * sizeof(float), hipMemcpyHostToDevice, mem_stream));
    CHECK_HIP(hipMalloc(&d_tile_sq_sums, h_tile_sq_sums.size() * sizeof(float)));
    CHECK_HIP(hipMemcpyAsync(d_tile_sq_sums, h_tile_sq_sums.data(), h_tile_sq_sums.size() * sizeof(float), hipMemcpyHostToDevice, mem_stream));
    CHECK_HIP(hipMalloc(&d_pixels, h_pixels.size() * sizeof(unsigned char)));
    CHECK_HIP(hipMemcpyAsync(d_pixels, h_pixels.data(), h_pixels.size() * sizeof(unsigned char), hipMemcpyHostToDevice, mem_stream));
    ...
    CHECK_HIP(hipStreamSynchronize(mem_stream));
    ...
    if (g_rank == 0) std::cout << "[Phase 3] Pipelined Execution with TurboJPEG..." << std::endl;
    ...
    for (size_t i = 0; i < preloaded_targets.size(); ++i) {
        ...
        CHECK_HIP(hipStreamSynchronize(res.stream));
        ...
        CHECK_HIP(hipMemcpyAsync(res.d_raw_input_img, res.h_pinned_input, in_img_bytes, hipMemcpyHostToDevice, res.stream));
        ...
        CHECK_ROCBLAS(rocblas_set_stream(blas_handle, res.stream));
        CHECK_ROCBLAS(rocblas_sgemm(
            blas_handle, rocblas_operation_transpose, rocblas_operation_none, M, N, K, &alpha, d_features, K, res.d_input_blocks, K, &beta, res.d_gemm_result, M));
        ...
        CHECK_HIP(hipMemcpyAsync(res.h_pinned_output, res.d_output_img, out_img_bytes, hipMemcpyDeviceToHost, res.stream));
    ...
    for (int i = 0; i < NUM_STREAMS; ++i) {
        CHECK_HIP(hipStreamSynchronize(streams[i].stream));
    ...
    CHECK_HIP(hipFree(d_features)); CHECK_HIP(hipFree(d_tile_sq_sums)); CHECK_HIP(hipFree(d_pixels));
    rocblas_destroy_handle(blas_handle); hipStreamDestroy(mem_stream);
